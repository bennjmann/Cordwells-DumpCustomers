...
namespace Cordwell.EmailService.API.Connector
{
    public class CGAPIConnector
    {
        ...
        public async Task<List<OrderRet>> ListOrdersByOrderDateAsync(DateTime _fromDate, DateTime _toDate)
        {
            //Ensure full date range with times.
            var fromDate = new DateTime(_fromDate.Year, _fromDate.Month, _fromDate.Day);
            var toDate = new DateTime(_toDate.Year, _toDate.Month, _toDate.Day, 23, 59, 59);
            ...
            var request = new RequestCommon<OrderQueryRequest>
            {
                WebcreteXMLMsgsRq = new WebcreteXMLMsgsRq<OrderQueryRequest>
                {
                    Request = new OrderQueryRequest
                    {
                        FromOrderDate = fromDate,
                        ToOrderDate = toDate,
                        IncludeRetElement = "USERDEFINEDFIELD"
                    }
                }
            };

            var test = Serialize(request);
            test = test.Replace("Request", "OrderQueryRq");
            // Console.WriteLine("Request XML:");
            // Console.WriteLine(test);

            if (isLoggedIn)
            {
                // Console.WriteLine(api.ProcessRequestAsync(_ticketHeader, test).Result.ProcessRequestResult);
                
                var rawResponse = await api.ProcessRequestAsync(_ticketHeader, test);
                var response = Deserialize<TicketQueryResponse>(rawResponse.ProcessRequestResult);
                
                var result = response.WebcreteXMLMsgsRs.OrderQueryRs.OrderRet.Where(x => x.OrderDate >= fromDate && x.OrderDate <= toDate).ToList();
                return result;
